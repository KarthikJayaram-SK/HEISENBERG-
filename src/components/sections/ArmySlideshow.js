...
const ArmySlideshow = () => {
  ...
  useEffect(() => {
    const fetchSlides = async () => {
      try {
        // ✨ This line fetches from the Army collection
        const q = query(collection(db, "armySlideshowImages"), orderBy("order", "asc"), limit(20));
        const querySnapshot = await getDocs(q);
        const fetchedSlides = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        setSlides(fetchedSlides);
      } catch (error) {
        console.error("Error fetching Army slideshow images:", error);
      }
    };
    fetchSlides();
  }, []);
  ...
  return (
    <SlideshowContainer>
      <SlideshowContent>
        <SectionTitle>Army Wing Gallery</SectionTitle>
        <CarouselWrapper>
          <AnimatePresence>
