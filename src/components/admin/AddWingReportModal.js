// src/components/admin/AddWingReportModal.js
...
const Select = styled.select` border: 1px solid #ddd; border-radius: 8px; padding: 0.75rem; font-family: inherit; font-size: 1rem; background: white; `;
const FileInput = styled.input` border: 1px solid #ddd; border-radius: 8px; padding: 0.5rem; font-family: inherit; font-size: 0.9rem; `; // Re-added FileInput style
...
const AddWingReportModal = ({ isOpen, onClose, onComplete }) => {
  const [title, setTitle] = useState('');
  // 2. Store the file object, not the URL
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // 4. Check for the file object
    if (!title || !pdfFile || !selectedWing) {
    ...
    try {
      ...
      setMessage('Wing report added successfully!');
      onComplete();
      setTimeout(onClose, 1500);
    } catch (error) {
      console.error("Error saving wing report:", error);
      ...
      {isOpen && (
        <ModalBackdrop initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <ModalContent initial={{ scale: 0.7 }} animate={{ scale: 1 }} exit={{ scale: 0.7 }}>
            <ModalHeader>
              <ModalTitle>Add New Wing Report</ModalTitle>
...
export default AddWingReportModal;